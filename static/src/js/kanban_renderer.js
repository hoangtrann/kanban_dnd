            ...
            attrs.forEach(function(item) {
                ...
                switch (item) {
                    case 'disable_drag_drop_record':
                        self.columnOptions.draggable = !JSON.parse(arch.attrs[item].toLowerCase());
                    case 'disable_sort_record':
                        self.recordOptions.sortable = !JSON.parse(arch.attrs[item].toLowerCase());
                    case 'disable_sort_column':
                        self.columnOptions.sortable = !JSON.parse(arch.attrs[item].toLowerCase());
                };
            });
        };
    },
